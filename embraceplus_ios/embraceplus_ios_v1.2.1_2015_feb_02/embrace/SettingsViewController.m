...
- (void)viewDidLoad
{
    ...
    viewControllers = [[NSArray alloc] initWithObjects:@"  Themes", /*@"  LANGUAGES",*/ @"  Notifications",
                       @"  Embrace+ battery",@"  About", @"  Help", @"  Theatre Mode", nil];
    
    indexImages = [[NSArray alloc] initWithObjects:@"styleIcon", /*@"languageIcon",*/ @"notificationIcon",@"icon-batteryStateEmbrace", @"aboutIcon", @"helpIcon",@"notificationIcon",nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if (indexPath.row == 2)
    {
        ...
        batteryLevelLabel.textAlignment = NSTextAlignmentRight;
        ...
        [batteryLevelLabel release];
            ...
            UISwitch *notificationSwitch = [[UISwitch alloc]init];
            [notificationSwitch addTarget:self action:@selector(theatreSwitch:) forControlEvents:UIControlEventValueChanged];
            CGRect frame = notificationSwitch.frame;
            frame.origin.x = 248.0;
            frame.origin.y = 10.0;
            notificationSwitch.frame = frame;
            ...
            NSNumber *globleNotificationSwitchStatus = [ud objectForKey:@"theatreMode"];
        BOOL noLight = [ud boolForKey:@"noLight"];
        if(!noLight) {
            noLight = false;
        }
            if(noLight == true)
...
- (IBAction)theatreSwitch:(id)sender {
    
    UISwitch *switchButton = (UISwitch*)sender;
    BOOL isButtonOn = [switchButton isOn];
    
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    [ud setObject:[NSNumber numberWithBool:isButtonOn] forKey:@"theatreMode"];
    [ud setBool:isButtonOn forKey:@"noLight"];
